// =============================================================================
// CLIENT LIFECYCLE ENGINE v2.0 - CORE BUS CONNECTED
// Consolidated: All lifecycle events published to ferunda-core-bus
// =============================================================================
...
async function publishToCoreBus(
  ...
  try {
    ...
    await channel.send({
      type: 'broadcast',
      event: eventType,
      payload: { ...payload, timestamp: Date.now(), source: 'client-lifecycle' }
    });
    console.log(`[ClientLifecycle] Published ${eventType} to Core Bus`);
  } catch (err) {
    console.error('[ClientLifecycle] Core Bus publish error:', err);
  }
}
