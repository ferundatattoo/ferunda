// =============================================================================
// CODESIGN ENGINE v2.0 - CORE BUS CONNECTED
// Consolidated: All codesign events published to ferunda-core-bus
// =============================================================================
...
async function publishToCoreBus(
  ...
  try {
    ...
    await channel.send({
      type: 'broadcast',
      event: eventType,
      payload: { ...payload, timestamp: Date.now(), source: 'codesign-engine' }
    });
    console.log(`[CoDesign] Published ${eventType} to Core Bus`);
  } catch (err) {
    console.error('[CoDesign] Core Bus publish error:', err);
  }
}
