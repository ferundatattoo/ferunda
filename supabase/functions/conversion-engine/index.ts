// =============================================================================
// CONVERSION ENGINE v2.0 - CORE BUS CONNECTED
// Consolidated: All conversion events published to ferunda-core-bus
// =============================================================================
...
async function publishToCoreBus(
  ...
  try {
    ...
    await channel.send({
      type: 'broadcast',
      event: eventType,
      payload: { ...payload, timestamp: Date.now(), source: 'conversion-engine' }
    });
    console.log(`[ConversionEngine] Published ${eventType} to Core Bus`);
  } catch (err) {
    console.error('[ConversionEngine] Core Bus publish error:', err);
  }
}
