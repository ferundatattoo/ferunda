// =============================================================================
// DESIGN ORCHESTRATOR v2.0 - CORE BUS CONNECTED
// Consolidated: All design pipeline events published to ferunda-core-bus
// =============================================================================
...
async function publishToCoreBus(
  ...
  try {
    ...
    await channel.send({
      type: 'broadcast',
      event: eventType,
      payload: { ...payload, timestamp: Date.now(), source: 'design-orchestrator' }
    });
    console.log(`[DesignOrchestrator] Published ${eventType} to Core Bus`);
  } catch (err) {
    console.error('[DesignOrchestrator] Core Bus publish error:', err);
  }
}
