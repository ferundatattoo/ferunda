// =============================================================================
// ANALYZE REFERENCE v2.0 - CORE BUS CONNECTED
// Consolidated: All reference analysis events published to ferunda-core-bus
// =============================================================================
...
async function publishToCoreBus(
  ...
  try {
    ...
    await channel.send({
      type: 'broadcast',
      event: eventType,
      payload: { ...payload, timestamp: Date.now(), source: 'analyze-reference' }
    });
    console.log(`[AnalyzeReference] Published ${eventType} to Core Bus`);
  } catch (err) {
    console.error('[AnalyzeReference] Core Bus publish error:', err);
  }
}
